...
class SarsaAgent:
    ...
    def get_state_from_observation(self, observation):
        ...
        if position <= -0.6 :
            x = 1
        if position <= -0.4 and position> -0.6:
            x = 2
        if velocity > 0 :
            y = 0
        if velocity <= 0 :
    ...
    def fit_step(self, s, a, r, new_state):
        ...
        self.Q[s[0],s[1],self.get_action_index(a)] += self.alpha*(r+ self.gamma*self.Q[new_state[0],new_state[1],self.get_action_index(next_action)] - self.Q[s[0],s[1],self.get_action_index(a)])
